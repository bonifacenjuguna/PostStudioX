...
- **That action is genuinely smart, not a relabeled resend**: it edits the live message in place whenever Telegram's API can express the change (text edit, caption edit, or media swap), and only falls back to delete-and-resend when the content type truly changed shape (e.g. text → photo) in a way no edit call can handle — the previous "Edit In Place" bug (silently calling the wrong API for text posts) is gone since this is a full rebuild of that mechanism, not a patch on top of it.
- **Found why error logs seemed to disappear**: every structured error message said "Also logged - Settings → Watchdog → Recent Events," but that screen only ever read the system-health log table, never the per-action error table `logAction` actually writes to. The promise was false the whole time. Recent Events now merges both sources.

## v2.2.1 — the "Custom Id" mystery, actually solved

- **Found the real cause of "Custom Id cannot contain :"**, with an authoritative source this time (BullMQ's own `Job.validateOptions`): a jobId containing a colon is only accepted if splitting on `:` gives exactly 3 parts. `post:15` (2 parts) was **always** invalid — this bug predates this whole rebuild, it just never got exercised against a real running worker until v2.2.0 actually started one. Every job ID in the codebase switched from colons to hyphens (`post-15`), matching BullMQ's own documented recommendation. Added a regression test that encodes BullMQ's actual rule directly, so this class of bug can't silently return.
- **Fixed Edit In Place silently not updating the live channel message**: `applyLiveEdit` called `editMessageCaption` unconditionally, which only works on media messages — calling it on a plain text post fails outright, and that failure was only ever logged to the console, never shown to the owner. The bot said "✅ Caption updated" regardless. Now branches correctly (editMessageText for text posts, editMessageCaption otherwise) and tells the owner plainly if the live message genuinely couldn't be updated, rather than claiming success either way.
